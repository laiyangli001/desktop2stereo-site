...
开放注册仍会受 Critical IP 限流保护，并要求完成服务端自托管拖动拼图验证码和邮箱验证。
登录同样要求一次性拖动拼图验证码；验证码由 `/api/captcha` 生成，默认 5 分钟有效，成功
提交后立即消费，不能跨请求重放。发送邮箱验证码、密码重置和签到等其他高风险入口仍沿用
各自现有的 Turnstile 防护。分布式 IP 多账号攻击不能仅靠单一验证码或 IP 限流阻止；更强的
设备或多维风控需作为独立安全项目设计。
